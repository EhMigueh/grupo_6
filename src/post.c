...
void publish_post(Post_List* post_list, const User* user, const char* content) {
    ...
    if (newPost) {
        ...
        printf(GREEN "Publicación creada con éxito!" RESET "\n");
    }
}
...
void displayUserPosts(const Post_List* post_list, int user_Id) {
    ...
    if (userPostCount == 0) {
        printf("No hay publicaciones para este usuario.\n");
    }
}
...
int likePost(Post_List* post_list, int post_Id) {
    ...
    while (current) {
        if (current->post_Id == post_Id) {
            current->likes++;
            printf(GREEN "Has dado like a la publicación %d" RESET "\n", post_Id);
            return 1;
        }
        current = current->next;
    }

    printf(RED "Publicación no encontrada." RESET "\n");
    return 0;
}
